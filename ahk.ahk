  ...
  Global indexFolders				; default search locations
  ...
  Join(select)
    ...
    Loop, Parse, select, `,
      if getMedia(A_LoopField)
        ...
        fullPath := inca . "\cache\temp\" . A_Index . "." . ext
        FileCopy, %src%, %fullPath%, 1
        escaped := StrReplace(fullPath, "'", "'\'")
        escaped := StrReplace(escaped, "\", "/")
        str .= "file '" escaped "'`r`n"
        }
    FileDelete, %inca%\cache\temp\temp.txt
    FileAppend, %str%, %inca%\cache\temp\temp.txt, UTF-8-RAW
    runwait, %inca%\cache\apps\ffmpeg.exe -f concat -safe 0 -i "%inca%\cache\temp\temp.txt" -c copy -y "%mediaPath%\%media% -join.%ext%",, Hide
    if ErrorLevel
...
if (ext == "txt")
  ...
  CutCopyPaste()
      ...
      if (value == "myPaste")
       if (InStr(ClipBoard, "#copy#") || InStr(ClipBoard, "#cut#"))
        {
        if InStr(ClipBoard, "#copy#")
          longClick = LButton
        ClipBoard := StrReplace(ClipBoard, "#copy#")
  ...
  Initialize()
   ...
   checkPlaylist()							; in case src files moved
     ...
     if A_LoopField
       ...
       start := StrSplit(A_Loopfield, "|").2
       detectMedia(source)
       searchSpace := searchFolders . "|" . indexFolders
       IfNotExist, %source%
         Loop, Parse, searchSpace, `|
...
;      media_s := "... " . media_s					; highlight as image (not video)
  ...
  TimedEvents:								; every 100mS
    ...
    if (!GetKeyState("LButton", "P"))
      IfWinActive, ahk_group Browsers
        if (h <= A_ScreenHeight - 4  &&  h >= A_ScreenHeight - 120)
          WinMove, ahk_group Browsers,, %wx%, -2, %w%, %y%
    IfWinActive, Notepad
      if (wx != 622)
