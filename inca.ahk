...
; text files fail to load to notepad because no longer inca internal ref "#%cap%#%i%"
    ...
    SpoolList(i, input, sort_name)							; spool sorted media files into web page
        {
        caption =
        start := 0
...
gap := Setting("Thumb Gap")
        ...
        if cap
            cap = <div style="width:70`%; margin-left:28`%; color:#826858; font-size:0.85em;"><span id="cap%i%">%cap%</span></div>
        if ((folder == "Favorites" || folder == "playlists") && type == "image")
        ...
        IfExist, %inca%\favorites\%media%*.*
            if (folder != "playlists" && folder != "Favorites")
  ...
  IfNotExist, %inca%\cache\thumbs\%media%.jpg
     poster = 
        if !view							; list view 
            ...
            else entry = <li onclick="select(%i%)" style="display:inline-block; vertical-align:top; width:%width%`%; margin-bottom:8em; color:%font%; transition:color 1.4s;"><div id="title%i%" style="width:70`%; margin-left:28`%; color:#555351; text-align:center; overflow:hidden; white-space:nowrap; text-overflow:ellipsis; font-size:0.85em; %highlight%;">%sort_name% &nbsp;&nbsp; %link% %media%</div><a href="#Media#%i%"><video id="media%i%" style="width:75`%; display:block; padding:2px; margin-left:6em; %transform% %select%" onmouseover="getLink(event, this, '%start%', '%type%', %i%)" onclick="open_media(event, this, '%start%', '%type%', %i%)" src="file:///%src%" %poster% muted type="video/mp4"></video></a>%cap%</li>`r`n`r`n
            }
        return entry
        }
    ...
    RenderPage()							; construct web page from media list
        ...
        Loop, Parse, list, `n, `r 					; split list into smaller web pages
            ...
            if ((list_size > (page-1) * size) && (list_size <= page * size))
                if (x := SpoolList(A_Index, source, sort_name))
                    {
...
; <link rel="stylesheet" type="text/css" href="https://fonts.googleapis.com/css?family=Open+Sans:300">`r`n
        ...
        panel_html = %panel2_html%<ul class="menu" id='panel' style="height:4.5em; width:80`%; margin-left:4em; margin-top:1em; margin-bottom:2em; column-count:8; border-radius:9px; padding-left:1em; font-size:0.9em"></ul>`r`n`r`n<ul class="menu" style="display:flex; justify-content:space-between"><a class='slider' id='sub' onmouseover='spool(event, id, "%subfolders%", "panel")' style="width:7`%;">Sub</a>`r`n<a href="file:///%inca%/cache/html/downloads.htm" class='slider' id='folders' onmouseover='spool(event, id, "%folder_list%", "panel")' style="width:7`%;">Fol</a>`r`n<a  href="file:///%inca%/cache/html/favorites.htm" class='slider' id='fav' onmouseover='spool(event, id, "%fav_folders%", "panel")' style="width:7`%;">Fav</a>`r`n<a href="file:///%inca%/cache/html/new.htm" class='slider' id='playlists' onmouseover='spool(event, id, "%playlists%", "panel")' style="width:7`%;">Playlists</a>`r`n<a href="file:///%inca%/cache/html/new.htm" class='slider' id='music' onmouseover='spool(event, id, "%music%", "panel")' style="width:7`%;">Music</a>`r`n<a id='search' class='slider' onmousemove='spool(event, id, "%search_list%", "panel")' onmousedown='spool(event,"all","%search_list%", "all")'>Search</a></ul>`r`n`r`n
...
;<span class="status2" id="stat2" onmouseover='s2(event,this)'></span>
;<span id="myCap" class="caption"><h2></h2></span>   <span id="myCap" class="caption"></span>
    ...
    ClickEvent()
        ...
        if (click == "LButton")
          ...
          if (timer > 350 && !selected && A_Cursor == "IBeam")		; mouse long click over text
              ...
              if (link == "Null" && timer > 350)			; clicked web page 'white space'
                  {
                  seek_overide := seek					; last seek time
                  orphan_media := last_media				; select last media
                  PlayMedia(0)						; play last media
                  }
              else if (link == "Media" && timer > 350)
                  {
                  sleep 20
                  send, {Pause}						; close modal
                  PlayMedia(0)						; use mpv universal player
                  }
              else if (link == "Media" && timer <= 350 && GetMedia(0))
                  {
                  if (type == "document")
                  PlayMedia(0)
                  }
              if (link != "Media" && selected)
                  FileTransfer(link)					; between folders or playlists
              else if (link == "Nulllllllllll")
                    {
                    click =
                    view ^= 1
                    page := 1
...
;  send, k								; java intercept - stop modal opening
            ...
            if (timer > 350)
                ...
                if (playlist && sort != "Alpha")			; playlist must be unsorted to edit
                    ...
                    sort = Alpha
                    CreateList(1)
                    PopUp("Playlist must be unfiltered",1200,0.5,0.5)
                    }
                else select()
                }
  ...
  select()
    ...
    MediaEvents()							; from timer every 100mS
        ...
        if (seek && ypos > A_ScreenHeight - 44)
            ...
            {
            if cue
                PopUp(Round((position - cue),1),0,0,0)
        ...
        if (position < x && position < 0.2 && xpos < A_ScreenWidth * 0.05)
            ...
            if (video_speed > 0.75)
                ...
                Popup(Round(video_speed,2),600,0.47,0.47)
                }
            }
        }
    ...
    WheelEvents()							; from timer every 100mS
        ...
        if (music_player && xpos < A_ScreenWidth * 0.1 && ypos > A_ScreenHeight * 0.9)
            ...
            Popup("Music Speed",800,0.47,0.47)
            }
        else if video_player
            {
            if (type == "image" && ext != "gif")    ; && magnify < 0)		; pan image
                ...
                video_speed -= 0.05
                    Popup(Round(video_speed,2),120,0.47,0.47)
...
;                IfExist, %inca%\cache\lists\%tab_name%.txt
...
;return
        ...
        if (arg1 && arg2 && InStr(sort_list, arg2))
            ...
            if s_path
                ...
                tab_name := folder
                if arg2
                    SplitPath, arg2,,,,tab_name				; playlists and music folder, use playlist name
...
;          Popup(popup,400,0,0)
    ...
    Spool(input, count)
        ...
        if (med := DecodeExt(ex))
            ...
            list_size += 1
            list = %list%%list_id%/%input%/%med%/%sort_name%`r`n
            }
        }
    ...
    FindSource()
        ...
        PopUp("no source found",600,0.5,0.5)
        }
    ...
    FileTransfer(x)
        ...
        if (type == "m3u")
            PopUp("Added",400,0,0)
        else PopUp("Move",400,0,0)
        if (!playlist && x == "Transfer")				; file move
            {
    ...
    PlayMedia(next)						; play 0 current, +1 next or -1 previous in list
        ...
        if (!seek || (timer > 350 && folder != "playlists"))
        ...
        if (thumbsheet && type == "video" && duration >= 20)
            Run %inca%\apps\mpv --video-zoom=-0.3 "%inca%\cache\thumbs\%media%.jpg"
...
WinActivate, ahk_group Browsers
   ...
   send, {Pause}{Pause}{Pause}{Pause}					; close modal
    ...
    AddHistory()
        {
        if (folder != "playlists" && !InStr(path, "\history\") && ext != "lnk" && !thumbsheet)
    ...
    AddFavorites()
        ...
        popup = Favorite
        if video_player
            PopUp(popup,0,0.47,0.47)
        else PopUp(popup,0,0,0)
        if !playlist
            play = %inca%\playlists\New.m3u
        else play := playlist
        if (type == "audio")
            {
